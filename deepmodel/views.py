...
from .forms import SlicesForm, SlicesFormdl
...
def get_slice_dl(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            path1 = 'DICOM/' + str(patient_id) + '/DL/original/' + str(slice) + '.png'
            path2 = 'DICOM/' + str(patient_id) + '/DL/tumors/' + str(slice) + '.png'

            paths = [path1, path2]
            #pdb.set_trace()

            return render(request, 'pngdl.html', {'paths':paths})
...
def start(request, patient_id):
    ...
    for i in range(len(predicted) - 1):
        if 1 not in predicted[i] and 1 in predicted[i + 1]:
            ...
            cancered = True
    print_out = 'Tumors detected at srarting indexe/s'
    ...
    for img_indx in range(len(predicted) - 1):
        ...
        m = np.uint8(cv2.normalize(m, None, 0, 255, cv2.NORM_MINMAX))
        #
        # v = predicted[img_indx + 1].reshape(256, 256)
        # v = np.uint8(cv2.normalize(v, None, 0, 255, cv2.NORM_MINMAX))

        # if 1 not in m and 1 in v:
        n = model_in[img_indx].reshape(256, 256)
        n = np.uint8(cv2.normalize(n, None, 0, 255, cv2.NORM_MINMAX))
        cv2.imwrite(path + '/original/' + str(img_indx) + '.png', n)
        cv2.imwrite(path + '/tumors/' + str(img_indx) + '.png', m)
        tumo.append(m)

    return render(request, 'slices_dl.html', {'patient_id': patient_id, 'sent': sent})
